...
# Delete one at a time @unittest.skip("delete this line to run the test") in tests/python_functions_test.py to start working on a test
# Run the tests: python3 run_tests.py
# Read the error and fix by writing a function in src/python_functions_practice.py
# The ONLY file you need to write code in is src/python_functions_practice.py
...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
    cube_volume = calculate_volume_of_cube(3)
  ...
  def test_reverse_string(self):
    ...
    self.assertEqual("dlroW olleH", reversed_string)

  #Given a value in farenheit, convert this into celsius.
  def test_fahrenheit_to_celsius(self):
    celsius_value = fahrenheit_to_celsius(30)
    self.assertEqual(-1.1, celsius_value)
